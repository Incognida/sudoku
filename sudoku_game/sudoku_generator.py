...
class Grid:
    ...
    def show(self):
        ...
        for i in range(self.n * self.n):
            table.append(tuple(self.table[i]))
        return table
...
def generate_sudokus():
    ...
    while iterator < example.n ** 4:
        i, j = random.randrange(0, example.n * example.n, 1), random.randrange(
            0, example.n * example.n, 1)  # Выбираем случайную ячейку
        if flook[i][j] == 0:  # Если её не смотрели
            ...
            i_solution = 0
            for solution in solve_sudoku((example.n, example.n),
                                                table_solution):
    ...
    return solved_table, example.show()